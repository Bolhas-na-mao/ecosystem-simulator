...
void Rabbit::update(World& world) {
    ...
    if(!grasses.empty()) {
        ...
        for(auto& grassData : grasses) {
            int dx = abs(grassData.x - myPos.x);
            int dy = abs(grassData.y - myPos.y);
            int distance = dx + dy;
        ...
        if(abs(dx) <= 1 && abs(dy) <= 1) {
            ...
            bool isGrassStillThere = target != nullptr && dynamic_cast<Grass*>(target) != nullptr;

            if(isGrassStillThere) {
